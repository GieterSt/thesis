...
def analyze_single_model(filepath):
    ...
    try:
        ...
        if isinstance(data, dict) and 'results' in data:
            ...
            print(f"✅ Loaded {len(results)} model responses (Claude format)")
        elif isinstance(data, list):
            # Other models format: [...]
            results = data
            print(f"✅ Loaded {len(results)} model responses (Standard format)")
        else:
    ...
    print(f"📊 Analysis complete: {metrics['performance_grade']}")
...
|-------|-------------|----------------|-----------|-------------------|"""
    ...
    for metrics in sorted_metrics:
        ...
        if metrics['ground_truth_analysis']:
            ...
            readme_content += f"\n| **{model_display}** | {api_success:.1f}% {'✅' if api_success >= 90 else '❌' if api_success < 50 else '⚠️'} | {hourly_success:.1f}% | {daily_mae:.1f} PPFD | {metrics['performance_grade']} |"
        else:
            readme_content += f"\n| **{model_display}** | {api_success:.1f}% {'✅' if api_success >= 90 else '❌'} | N/A | N/A | {metrics['performance_grade']} |"
...
"""
    ...
    print(f"\n📝 README.md updated with comprehensive analysis!")
    print(f"📊 {len(all_metrics)} models included")
...
<div class="timestamp">
    ...
    📊 Research analysis automatically updated from model results
</div>
</body>
</html>'''
...
def run_comprehensive_analysis():
    ...
    if not result_files:
        print("📁 No result files found in results/model_outputs/")
        return
    
    print("🚀 COMPREHENSIVE ANALYSIS WITH README + HTML GENERATION")
    print("=" * 80)
    ...
    # 🎯 NEW: Run comprehensive statistical analysis
    print("\n" + "="*80)
    print("🎯 RUNNING THESIS-GRADE STATISTICAL ANALYSIS")
    print("="*80)
    
    stats_results = comprehensive_statistical_analysis(all_metrics)
    
    # 🎨 NEW: Generate thesis-quality visualizations
    ...
    print(f"📋 Master analysis: {master_file}")
    print(f"📝 README.md updated automatically")
    if html_success:
        print(f"🌐 HTML updated: docs/LLM_LED_Optimization_Research_Results.html")
    print(f"📊 {len(all_metrics)} models analyzed")
...
def monitor_and_auto_update():
    ...
    print("👁️ MONITORING: results/model_outputs")
    print("🔄 Will auto-analyze and update README + HTML when new results appear...")
    print("📝 README.md + HTML will be automatically updated with each new model")
    print("⏹️ Press Ctrl+C to stop monitoring")
    
    last_files = set()
    
    # Initial analysis
    print("\n📋 Running initial comprehensive analysis...")
    ...
    while True:
        try:
            ...
            if new_files:
                ...
                print("\n🔄 Running comprehensive analysis and updating README + HTML...")
                run_comprehensive_analysis()
                
                last_files = current_files
                print(f"✅ README.md + HTML updated with new results!")
...
def comprehensive_statistical_analysis(all_metrics):
    """
    🎯 THESIS-GRADE STATISTICAL ANALYSIS
    Performs comprehensive statistical tests for academic validation
    """
    print("\n" + "="*80)
    print("📊 COMPREHENSIVE STATISTICAL ANALYSIS FOR THESIS")
    print("="*80)
    
    if len(all_metrics) < 2:
        print("⚠️ Need at least 2 models for statistical analysis")
        return {}
    ...
    if len(model_data) < 2:
        print("⚠️ Need at least 2 models with ground truth analysis")
        return {}
    ...
    print(f"📊 Linear Regression: Success = {reg.coef_[0]:.3f} × log10(Params) + {reg.intercept_:.3f}")
    print(f"   R² = {r2:.3f}")
    print(f"   Slope: β = {reg.coef_[0]:.3f} ± {se_beta:.3f} (SE)")
